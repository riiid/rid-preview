...
class RidRenderer
  ...
  parsePart5: (html, node) ->
    ...
    for child in children
      ...
      switch child.name()
        when 'text' \
          then @makeTextOfPart5 html, root, child
        when 'list' \
          then @makeChoiceList html, root, child
        when 'level' \
          then @makeLevel html, root, child
        when 'correct' \
          then @makeCorrect html, root, child
        when 'pattern' \
          then @makePattern html, root, child
        when 'interpretation' \
          then @makeInterpretation html, root, child
        when 'explanation' \
          then @makeExplanation html, root, child
        when 'vocalist' \
          then @makeVocaList html, root, child
    root
  ...
  parsePart6: (html, node) ->
    ...
    for child in children
      ...
      switch child.name()
        when 'intro' \
          then @makeIntroPart html, root, child
        when 'text' \
          then @makeTextOfPart6 html, root, child
        when 'interpretation' \
          then @makeInterpretation html, root, child
        when 'vocalist' \
          then @makeVocaList html, root, child
        when 'answers' \
          then @makeAnswers html, root, child
    root
...
exports.toDOMFragment = (text='', filePath, grammar, callback) ->
  render text, filePath, (error, html) ->
    ...
    html = new RidRenderer().render(html, { noblanks: true })
